...
#    print(labels[m])
    ...
    for num in range(0,m) :
        print(num)
        if labels[num].endswith("wd"):
            labels[num] = "0"
        if labels[num].endswith("wgh"):
            labels[num] = "1"
        if labels[num].endswith("wjf"):
            labels[num] = "2"
        if labels[num].endswith("ymr"):
            labels[num] = "3"
...
if __name__ == '__main__':
    if len(sys.argv) != 1:
        ...
        images, labels = load_dataset("D:\\ai_projects\\face_detection\\cnn\\data")